...
export const migratePrioritiesToNewValues = async () => {
  ...
  await pool.query(
    `UPDATE tickets SET priority = 'Alta' WHERE priority IN ('alto', 'urgente')`,
  );
  await pool.query(
    `UPDATE tickets SET priority = 'Media' WHERE priority = 'medio'`,
  );
  await pool.query(
    `UPDATE tickets SET priority = 'Baja' WHERE priority = 'bajo'`,
  );
...
export const createTicket = async (
  ...
  if (!VALID_PRIORITIES.includes(priority as Priority)) {
    throw new Error(
      `Prioridad inválida. Valores permitidos: ${VALID_PRIORITIES.join(", ")}`,
    );
  }
  const result = await pool.query(
...
export const assignTicketToAgent = async (
  ...
  const agent = await pool.query("SELECT id, role FROM users WHERE id = $1", [
    agentId,
  ]);
...
export const reopenTicket = async (
  ...
  try {
    ...
    if (ticket.status !== "Cerrado")
      throw new Error('Solo se pueden reabrir tickets con estado "Cerrado"');
